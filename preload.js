...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const checkCurrentTimeSlot = (asString = false) => {
    ...
    return asString
      ? `${getHours12Format()}:${formatMinutes}`
      : Number(`${getHours12Format()}${formatMinutes}`)
  }
  ...
  const removeTimeSlot = (slot, slotToNumber) => {
    const indexToRemove = activeTimeSlots.findIndex(
      (e) => e.slot === slotToNumber
    )
